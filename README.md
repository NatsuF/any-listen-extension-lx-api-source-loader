...
| `.github/workflows/release.yml` | 改为 `master-quality` 分支触发、签名密钥预检、类型检查、上传构件 |
...
1. 到 Releases 下载 `lx-api-source-loader_v0.1.8.alix`，在 any-listen 中手动安装。
...
仓库 Secrets 需配置 `PRI_KEY` / `PUB_KEY`（Ed25519 PEM），否则 `extension-kit` 会拒绝打包。
